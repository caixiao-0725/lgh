...
void GLFWInit() {
 ...
 // ------------------------------
    ...
    // --------------------
    window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Crush on you ohhh", NULL, NULL);
    if (window == NULL)
    {
...
void createPBO() {
    ...
    glBufferData(GL_PIXEL_UNPACK_BUFFER, sizeof(GLubyte)*SCR_HEIGHT*SCR_WIDTH , 0, GL_DYNAMIC_COPY);
...
void createTexture()
{
    ...
    cudaGraphicsGLRegisterImage(&cuda_texture_resource, textureID, GL_TEXTURE_2D, cudaGraphicsRegisterFlagsWriteDiscard);
}
...
void createShader() {
    ...
    vShaderFile.open("shader/v.vs");
    fShaderFile.open("shader/f.fs");
    ...
    if (numLevels > 1) {
        ...
        for (int level = 1; level < numLevels - 1; level++) {
            ...
            levels[level].pc.GetPoints(position, r * 2, [&](int i, Vec3f& p, float dist2, float& radius2) {
                ...
                float d = sqrtf(dist2);
                if (i >= levels[level].pc.pointCount) return;
...
__global__ void testKernal(cudaSurfaceObject_t outputSurfObj,int width, int height,BallLight* balls,BVHNode* bvh, Level* levels, int numLevels, float cellSize, BVHNode* model_bvh,Triangle* triangles)
{
    ...
    if (x < width && y < height) {
        ...
        vec3f ro = vec3f(-0.4, 1.1, 0.9);
        vec3f rd_ = vec3f(0.28, -0.4f, -1) + ((float)x / SCR_WIDTH - 0.5f) * vec3f(1.5, 0, 0) + ((float)y - SCR_HEIGHT / 2) / SCR_WIDTH * vec3f(0, 1.5, 0);
        ...
        temp_res = ray.hitWalls(vec3f(-1.1f,0.0f,-1.5f),vec3f(1.1f,5.0f,3.0f));
        if (temp_res.IsHit) {
            res = temp_res;
        ...
        if (res.IsHit) {
            ...
            col.clamp(0.0f, 1.0f);
            //vec3f col = levels[].colors[3];
            data.x = int(col.x * 255);
            data.y = int(col.y * 255);
            data.z = int(col.z * 255);
        ...
        // Write to output surface  虽然划红线但是没有关系
...
int main()
{
    ...
    for (int i = 0; i < diff_num; i++) {
        balls_host[i].position = vec3f(2.0f*rand() / RAND_MAX-1.0f, 0.5f*float(rand()) / RAND_MAX+0.6f, -((0.5f * rand() / RAND_MAX)+0.8f));
    ...
    for (int i = 0; i < BallNum - diff_num; i++) {
        balls_host[i+diff_num].position = vec3f(1.9f * rand() / RAND_MAX - 1.0f, 1.2f * float(rand()) / RAND_MAX + 1.1f, -((0.5f * rand() / RAND_MAX) + 0.8f));
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glBindVertexArray(vao);
        cudaGraphicsMapResources(1, &cuda_texture_resource);
        cudaGraphicsSubResourceGetMappedArray(&devPtr,cuda_texture_resource,0,0);
        cudaResourceDesc viewCudaArrayResourceDesc;
        {
            viewCudaArrayResourceDesc.resType = cudaResourceTypeArray;
            viewCudaArrayResourceDesc.res.array.array = devPtr;
        }
        ...
        {   
            ...
            testKernal<<<grid,block>>>(viewCudaSurfaceObject,SCR_WIDTH,SCR_HEIGHT,balls_device,nodes_device, levels_device, numLevels, nodeCellSize, model_bvh_device,tri_device);
        }
        cudaDestroySurfaceObject(viewCudaSurfaceObject);
        cudaGraphicsUnmapResources(1, &cuda_texture_resource);
        //glBindTexture(GL_TEXTURE_2D, textureID);
