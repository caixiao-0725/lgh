...
struct vec3
{
	...
	__host__ __device__ inline void clamp(float min, float max){
		if (x < min) {
			x = min;
		}
		else if (x > max) {
	...
	__host__ __device__ inline T Min() { return fminf(x, fminf(y, z)); }
	__host__ __device__ inline T Max() { return fmaxf(x, fmaxf(y, z)); }
	__host__ __device__ inline T dot(const vec3<T>& temp) { return x * temp.x + y * temp.y + z * temp.z; }
	__host__ __device__ inline vec3<int> operator << (const int temp) { return vec3<int>(x << temp, y << temp, z << temp); }
};
template <typename T>
__host__ __device__ inline vec3<T> operator * (const T a,vec3<T>& temp) { return temp * a; }
template <typename T>
__host__ __device__ inline vec3<T> operator / (T a, vec3<T>& temp) { return vec3f(a / temp.x, a / temp.y, a / temp.z); }
template <typename T>
__device__ inline vec3<T> Min(vec3<T>& temp, vec3<T>& temp_) {return vec3f(fminf(temp.x, temp_.x), fminf(temp.y, temp_.y), fminf(temp.z, temp_.z));}
template <typename T>
__device__ inline vec3<T> Max(vec3<T>& temp, vec3<T>& temp_) { return vec3f(fmaxf(temp.x, temp_.x), fmaxf(temp.y, temp_.y), fmaxf(temp.z, temp_.z)); }
